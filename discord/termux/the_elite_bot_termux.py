import discord, asyncio, os, shlex
...
try:os.system(f"""{llama_main_path} -m {model_path} -p '{preprompt}' -n 1 -c 2048 -e -t {threads} --prompt-cache {cache_file} -r '###' > /dev/null 2>&1""")
except:pass
...
async def on_message(message):
    ...
    if message.author == client.user:
        ...
        command = f"{llama_main_path} -m {model_path} -p '{prompt}' -n 500 -c 2048 -e -t {threads} --prompt-cache-all --prompt-cache {cache_file} -r '###'"
        async with message.channel.typing():
            ...
            while True:
                ...
                if 'llama_print_timings:' in completion or 'main: saving' in completion:
                    completion = completion.split('llama_print_timings:')[0]
                    completion = completion.split('main: saving')[0]
